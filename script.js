...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('mouseover', e => {
        const interactive = e.target.closest('a, button, select, input, textarea, .lang-option, .cb-container, .modal-close');
        if (interactive) {
    ...
    const filterButtons = document.querySelectorAll('#panel-marketplace button[data-filter]');
    const assetCards = document.querySelectorAll('#panel-marketplace .asset-card');
    ...
    if (filterButtons && assetCards) {
        filterButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                assetCards.forEach(card => {
                    ...
                    if (filterValue === 'All' || categories.some(cat => cat.trim().toLowerCase() === filterValue.toLowerCase())) {
        ...
        const defaultActive = document.querySelector('#panel-marketplace button[data-filter].active');
        if (defaultActive) {
    ...
    if (termsModal && privacyModal) {
        document.addEventListener('click', (e) => {
            ...
            } else if (target.closest('#privacyLink') || target.closest('#cookieLearnMore')) {
                e.preventDefault();
                showModal(privacyModal);
            }
        });
        
        document.querySelectorAll('.ft-lnks a, .modal-trigger').forEach(trigger => {
    ...
    if (accessForm) {
        accessForm.addEventListener('submit', (e) => {
            ...
            console.log("Submitting Request Payload to EquusChain Registry:", payload);
            ...
            .then(data => {
                ...
                setTimeout(() => {
                    ...
                    let bodyText = "Thank you for your inquiry. Our relations desk will review your credentials and contact you directly.";
                    
                    if (currentLang === 'fr') {
                        titleText = "Demande d'accès reçue";
                        bodyText = "Nous vous remercions pour votre intérêt. Notre bureau des relations examinera vos justificatifs et vous contactera directement.";
                    } else if (currentLang === 'ar') {
                        titleText = "تم استلام طلب الانضمام";
                        bodyText = "نشكرك على استفسارك. سيقوم مكتب العلاقات لدينا بمراجعة أوراق اعتمادك والتواصل معك مباشرة.";
                    } else if (currentLang === 'es') {
                        titleText = "Solicitud de acceso recibida";
                        bodyText = "Gracias por su consulta. Nuestra oficina de relaciones revisará sus credenciales y se pondrá en contacto con usted directamente.";
                    } else if (currentLang === 'de') {
                        titleText = "Zugangsanfrage erhalten";
                        bodyText = "Vielen Dank für Ihre Anfrage. Unser Büro für Kundenbeziehungen wird Ihre Angaben prüfen und sich direkt mit Ihnen in Verbindung setzen.";
                    } else if (currentLang === 'it') {
                        titleText = "Richiesta di accesso ricevuta";
                        bodyText = "Grazie per la richiesta. Il nostro ufficio relazioni esaminerà le sue credenziali e la contatterà direttamente.";
                    } else if (currentLang === 'pt') {
                        titleText = "Pedido de acesso recebido";
                        bodyText = "Agradecemos o seu contacto. A nossa equipa de relações avaliará as suas credenciais e entrará em contacto consigo diretamente.";
                    } else if (currentLang === 'ru') {
                        titleText = "Запрос доступа получен";
                        bodyText = "Благодарим вас за обращение. Наш отдел по связям с клиентами рассмотрит ваши данные и свяжется с вами напрямую.";
                    } else if (currentLang === 'zh') {
                        titleText = "已收到访问申请";
                        bodyText = "感谢您的咨询。我们的客户服务部门将审核您的资质，并直接与您联系。";
                    } else if (currentLang === 'hi') {
                        titleText = "अनुरोध प्राप्त हुआ";
                        bodyText = "पूछताछ के लिए धन्यवाद। हमारा डेस्क आपके क्रेडेंशियल्स की समीक्षा करेगा और आपसे सीधे संपर्क करेगा।";
                    } else if (currentLang === 'ja') {
                        titleText = "アクセス申請完了";
                        bodyText = "申請を受け付けました。担当部門にて資格要件を確認の上、直接ご連絡いたします。";
                    }
...
const translations = {
    en: {
        ...
        "hero-lead": "EquusChain is the modern infrastructure for the ownership, exchange, and liquidity of exceptional real-world and traditional finance assets.",
        "manifesto-tag": "I. Our Conviction",
        "manifesto-body": "The world's most enduring assets have always transcended markets and generations. From private enterprises to exceptional real estate, to distinguished bloodstock and fine art, their value extends beyond financial performance alone. EquusChain provides the infrastructure to manage, exchange, and unlock liquidity in these assets with the discretion, transparency, and efficiency they deserve.",
        "universe-subtitle": "II. Asset Universe",
        "universe-title": "EquusChain Universe of<br>Exceptional Assets.",
        "universe-lead": "An extensive and growing universe. From institutional private markets to the world's most coveted real-world assets. A selection below the full catalogue lives inside the platform.",
        "asset-1-name": "Real Estate & Infrastructure",
        "asset-1-sub": "Fractional or whole.",
        ...
        "services-lead": "Four things. Each designed for a specific kind of relationship with capital. You may need one. You may need all four.",
        "service-1-n": "I",
        "service-1-title": "Invest & Trade",
        "service-1-body": "Subscribe to offerings. Manage positions. Receive distributions. One private environment for a portfolio that extends far beyond listed markets.",
        "service-1-link": "Open an account",
        "service-2-n": "II",
        "service-2-title": "Fractional Ownership",
        "service-2-body": "Hold interests in assets that would otherwise demand exclusive capital commitment. Real estate, private enterprises, bloodstock, aircraft, and many more; at the size that suits your mandate.",
        "service-2-link": "Enquire",
        "service-3-n": "III",
        "service-3-title": "Bring Assets to Market",
        "service-3-body": "You hold the asset. We structure it, document it, and bring it to qualified investors. From your privately held company to a property portfolio or a yacht; the entire process, handled.",
        "service-3-link": "Speak with our team",
        "service-4-n": "IV",
        "service-4-title": "Access & Use Rights",
        "service-4-body": "Certain platform assets are available through preferential access arrangements. Aviation, maritime, equestrian. Reserved for members.",
        "service-4-link": "Explore",
        "contact-subtitle": "IV. Access",
        "contact-title": "Request<br><em>Access.</em>",
        "contact-body": "Every inquiry is reviewed directly by our team to ensure the quality, relevance, and integrity of the EquusChain ecosystem.",
        ...
        "opt-interest-8": "General Inquiry",
        "form-note-label": "A brief note (optional)",
        "form-newsletter-text": "I would like to subscribe to EquusChain's curated newsletter",
        "form-consent-text": "I confirm that I have read and agree with the <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Terms &amp; Conditions</a> and <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Privacy Policy</a>",
        "form-submit-btn": "Submit Request",
        "form-note-footer": "Your enquiry is handled with complete discretion. We do not share details with third parties.",
        ...
        "cookie-body": "We use cookies to improve your experience, analyze site usage, and, if you wish, to offer you personalized content and offers. Some cookies are essential for the site to function (session, security). Other cookies (analytics, marketing, tracking) require your prior consent.",
        ...
        "login-register-note": "Access is subject to KYC/AML verification and investor eligibility assessment. Our team will be in contact within 24–48 hours.",
    ...
    fr: {
        ...
        "hero-title": "Les plus beaux<br>actifs du monde,<br><em>enfin</em> liquides.",
        "hero-lead": "EquusChain is l'infrastructure moderne pour la détention, l'échange et la liquidité d'actifs réels exceptionnels et de finance traditionnelle.",
        "manifesto-tag": "I. Notre Conviction",
        "manifesto-body": "Les actifs les plus durables du monde ont toujours transcendé les marchés et les générations. Des entreprises privées à l'immobilier d'exception, en passant par l'élevage distingué et les beaux-arts, leur valeur va au-delà de la seule performance financière. EquusChain fournit l'infrastructure pour gérer, échanger et libérer la liquidité de ces actifs avec la discrétion, la transparence et l'efficacité qu'ils méritent.",
        "universe-subtitle": "II. Univers des Actifs",
        "universe-title": "Univers d'Actifs<br>Exceptionnels EquusChain.",
        "universe-lead": "Un univers vaste et en pleine croissance. Des marchés privés institutionnels aux actifs réels les plus convoités du monde. Une sélection ci-dessous le catalogue complet se trouve dans la plateforme.",
        "asset-1-name": "Immobilier & Infrastructure",
        "asset-1-sub": "Fractionné ou entier.",
        "asset-2-name": "Private Equity",
        ...
        "asset-4-sub": "Terres productives et instruments liés aux matières premières.",
        ...
        "asset-7-sub": "Œuvres de qualité muséale, vins rares, automobiles classiques.",
        "asset-8-name": "Et bien plus encore",
        "asset-8-sub": "Métaux précieux, fonds structurés, énergies renouvelables…",
        "services-subtitle": "III. Services",
        "services-title": "Ce que la plateforme<br>EquusChain rend<br><em>Possible.</em>",
        "services-lead": "Quatre services. Chacun conçu pour une relation spécifique avec le capital. Vous pouvez en avoir besoin d'un. Vous pouvez avoir besoin des quatre.",
        "service-1-n": "I",
        "service-1-title": "Investir & Échanger",
        "service-1-body": "Souscrivez aux offres. Gérez vos positions. Recevez les distributions. Un environnement privé unique pour un portefeuille qui s'étend bien au-delà des marchés cotés.",
        "service-1-link": "Ouvrir un compte",
        "service-2-n": "II",
        "service-2-title": "Propriété Fractionnée",
        "service-2-body": "Détenez des participations dans des actifs qui exigeraient autrement un engagement de capital exclusif. Immobilier, entreprises privées, élevage, avions, et bien plus encore ; à la taille qui convient à votre mandat.",
        "service-2-link": "S'informer",
        "service-3-n": "III",
        "service-3-title": "Introduire des Actifs sur le Marché",
        "service-3-body": "Vous détenez l'actif. Nous le structurons, le documentons et le présentons aux investisseurs qualifiés. De votre entreprise privée à un portefeuille immobilier ou un yacht ; l'intégralité du processus est gérée.",
        "service-3-link": "Parler avec notre équipe",
        "service-4-n": "IV",
        "service-4-title": "Droits d'Accès & d'Usage",
        "service-4-body": "Certains actifs de la plateforme sont disponibles via des accords d'accès préférentiels. Aviation, maritime, équestre. Réservé aux membres.",
        "service-4-link": "Explorer",
        "contact-subtitle": "IV. Accès",
        "contact-title": "Demander<br><em>l'Accès.</em>",
        "contact-body": "Chaque demande est examinée directement par notre équipe afin de garantir la qualité, la pertinence et l'intégrité de l'écosystème EquusChain.",
        ...
        "form-newsletter-text": "Je souhaite m'abonner à la newsletter d'EquusChain",
        "form-consent-text": "Je confirme avoir lu et accepté les <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Conditions Générales</a> et la <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Politique de Confidentialité</a>",
        "form-submit-btn": "Envoyer la demande",
        "form-note-footer": "Votre demande est traitée avec une discrétion absolue. Nous ne partageons aucune coordonnée avec des tiers.",
        ...
        "cookie-body": "Nous utilisons des cookies pour améliorer votre expérience, analyser l'utilisation du site et vous proposer des offres adaptées. Certains cookies sont essentiels pour le fonctionnement (sécurité, session). D'autres cookies nécessitent votre consentement préalable.",
        ...
        "login-note": "Accès sécurisé · Toutes les sessions sont surveillées et chiffrées",
        ...
        "login-register-note": "L'accès est soumis à la vérification KYC/AML et à l'évaluation de l'éligibilité de l'investisseur. Notre équipe vous contactera sous 24 à 48 heures.",
    ...
    ar: {
        ...
        "hero-lead": "إيكوس تشين هي البنية التحتية الحديثة لملكية وتبادل وسيولة الأصول الاستثنائية الحقيقية والمالية التقليدية.",
        "manifesto-tag": "١. قناعتنا",
        "manifesto-body": "إن الأصول الأكثر ديمومة في العالم لطالما تجاوزت الأسواق والأجيال. من الشركات الخاصة إلى العقارات الاستثنائية، إلى الخيول العربية الأصيلة والفنون الجميلة، قيمتها تمتد إلى ما هو أبعد من الأداء المالي وحده. توفر إيكوس تشين البنية التحتية لإدارة وتبادل وفتح السيولة في هذه الأصول بالسرية والشفافية والكفاءة التي تستحقها.",
        "universe-subtitle": "٢. عالم الأصول",
        "universe-title": "عالم الأصول الاستثنائية<br>من إيكوس تشين.",
        "universe-lead": "عالم واسع ومتنامي. من الأسواق الخاصة المؤسسية إلى الأصول الحقيقية الأكثر طلباً في العالم. مجموعة مختارة أدناه الكتالوج الكامل متاح داخل المنصة.",
        "asset-1-name": "العقارات والبنية التحتية",
        "asset-1-sub": "جزئية أو كاملة.",
        ...
        "services-lead": "أربعة أشياء. تم تصميم كل منها لنوع محدد من العلاقة مع رأس المال. قد تحتاج إلى واحدة. قد تحتاج إلى الأربعة معاً.",
        "service-1-n": "١",
        "service-1-title": "الاستثمار والتداول",
        "service-1-body": "الاكتتاب في الطروحات. إدارة المراكز المفتوحة. استقبال التوزيعات. بيئة خاصة واحدة لمحفظة تمتد إلى ما هو أبعد من الأسواق المدرجة.",
        "service-1-link": "افتح حساباً",
        "service-2-n": "٢",
        "service-2-title": "الملكية الجزئية",
        "service-2-body": "امتلاك حصص في أصول تتطلب بخلاف ذلك التزاماً مالياً حصرياً ضخماً. العقارات، الشركات الخاصة، الخيول، الطائرات، وغيرها الكثير؛ بالحجم الذي يناسب تفويضك المالي.",
        "service-2-link": "استفسر الآن",
        "service-3-n": "٣",
        "service-3-title": "طرح الأصول في السوق",
        "service-3-body": "أنت تملك الأصل. نحن نهيكله، ونوثقه، ونطرحه للمستثمرين المؤهلين. بدءاً من شركتك الخاصة وحتى المحفظة العقارية أو اليخت؛ نتولى العملية بأكملها.",
        "service-3-link": "تحدث مع فريقنا",
        "service-4-n": "٤",
        "service-4-title": "حقوق الوصول والاستخدام",
        "service-4-body": "بعض أصول المنصة متاحة من خلال ترتيبات وصول تفضيلية مخصصة للأعضاء. الطيران، الملاحة، الفروسية.",
        "service-4-link": "استكشف",
        "contact-subtitle": "٤. الانضمام",
        "contact-title": "طلب<br><em>الدخول.</em>",
        "contact-body": "تتم مراجعة كل استفسار مباشرة من قبل فريقنا لضمان جودة وموثوقية ونزاهة منظومة إيكوس تشين.",
        ...
        "form-newsletter-text": "أود الاشتراك في النشرة الإخبارية المخصصة لإيكوس تشين",
        "form-consent-text": "أؤكد أنني قرأت وأوافق على <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">الشروط والأحكام</a> و<a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">سياسة الخصوصية</a>",
        "form-submit-btn": "إرسال الطلب",
        "form-note-footer": "يتم التعامل مع استفسارك بسرية تامة. نحن لا نشارك التفاصيل مع أطراف ثالثة.",
        ...
        "ft-disclaimer": "© ٢٠٢٦ شركة إيكوس تشين المحدودة. جميع الحقوق محفوظة. المعلومات الواردة في هذا الموقع هي لأغراض إعلامية فقط ولا تشكل نصيحة استثمارية أو قانونية أو ضريبية أو مالية، كما لا تمثل عرضاً أو التماساً لشراء أو بيع أي ورقة مالية أو أداة مالية أو أصل. قد يكون الدخول إلى بعض الخدمات مقيداً بناءً على الولاية القضائية وتصنيف المستثمر والقوانين المعمول بها. ينطوي الاستثمار على مخاطر، بما في ذلك احتمال خسارة رأس المال.",
        "ft-privacy": "الخصوصية",
        "ft-terms": "الشروط",
        "cookie-title": "نحن نستخدم الكوكيز",
        "cookie-body": "نحن نستخدم ملفات تعريف الارتباط لتحسين تجربتك وتحليل الاستخدام وتقديم محتوى مخصص. بعض ملفات تعريف الارتباط ضرورية لتشغيل الموقع (الأمان والجلسة) بينما تتطلب الملفات الأخرى موافقتك المسبقة.",
        ...
        "login-register-note": "يخضع الدخول لإجراءات التحقق من الهوية (KYC/AML) وتقييم أهلية المستثمر. سيتواصل فريقنا معك خلال 24-48 ساعة.",
    ...
    it: {
        ...
        "hero-lead": "EquusChain è l'infrastruttura moderna per la proprietà, lo scambio e la liquidità di attivi reali eccezionali e finanza tradizionale.",
        "manifesto-tag": "I. La nostra convinzione",
        "manifesto-body": "I beni più durevoli del mondo hanno sempre superato i mercati e le generazioni. Dalle imprese private all'immobiliare di prestigio, ai cavalli da corsa selezionati e alle belle arti, il loro valore va oltre la sola performance finanziaria. EquusChain fornisce l'infrastruttura per gestire, scambiare e sbloccare la liquidità di questi beni con la discrezione, trasparenza ed efficienza che meritano.",
        "universe-subtitle": "II. Universo degli attivi", 
        "universe-title": "Universo degli attivi<br>eccezionali EquusChain.",
        "universe-lead": "Un universo ampio e in crescita. Dai mercati privati istituzionali ai beni reali più ambiti al mondo. Di seguito una selezione, il catalogo completo si trova nella plataforma.",
        "asset-1-name": "Immobiliare e Infrastrutture", 
        "asset-1-sub": "Frazionato o intero.",
        "asset-2-name": "Private Equity", 
        ...
        "services-lead": "Quattro servizi. Ciascuno progettato per un tipo specifico di relazione con il capitale. Potresti averne bisogno di uno o di tutti e quattro.",
        "service-1-n": "I", 
        "service-1-title": "Investire e Scambiare", 
        "service-1-body": "Sottoscrivi le offerte. Gestisci le posizioni. Ricevi le distribuzioni. Un ambiente privato per un portafoglio che va ben oltre i mercati quotati.", 
        "service-1-link": "Apri un conto",
        "service-2-n": "II", 
        "service-2-title": "Proprietà Frazionata", 
        "service-2-body": "Partecipa a beni che altrimenti richiederebbero impegni di capitale esclusivi. Immobiliare, imprese private, allevamenti, aerei ed altro; alla misura adatta al tuo mandato.", 
        "service-2-link": "Chiedi informazioni",
        "service-3-n": "III", 
        "service-3-title": "Portare beni sul mercato", 
        "service-3-body": "Tu possiedi il bene. Noi lo strutturiamo, documentiamo e presentiamo a investitori qualificati. Dalla tua azienda privata a un portafoglio immobiliare o un yacht; gestiamo l'intero processo.", 
        "service-3-link": "Parla con il nostro team",
        "service-4-n": "IV", 
        "service-4-title": "Diritti di accesso e d'uso", 
        "service-4-body": "Alcuni attivi della piattaforma sono disponibili tramite accordi di accesso preferenziale. Aviazione, nautica, equitazione. Riservato ai membri.", 
        "service-4-link": "Esplora",
        "contact-subtitle": "IV. Accesso", 
        "contact-title": "Richiedi<br><em>l'Accesso.</em>",
        "contact-body": "Ogni richiesta viene esaminata direttamente dal nostro team per garantire la qualità, pertinenza e integrità dell'ecosistema EquusChain.",
        ...
        "form-consent-text": "Confermo di aver letto e accettato i <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Termini e Condizioni</a> e l'<a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Informativa sulla Privacy</a>", 
        "form-submit-btn": "Invia richiesta", 
        "form-note-footer": "La tua richiesta è gestita con assoluta discrezione. Non condividiamo i dettagli con terze parti.",
        ...
        "cookie-body": "Utilizziamo i cookie per migliorare l'esperienza, analizzare l'uso del sito e offrire contenuti personalizzati. Alcuni cookie sono essenziali per il funzionamento (sicurezza, sessione). Altri richiedono il tuo preventivo consenso.",
        ...
        "login-note": "Accesso protetto · Tutte le sessioni sono monitorate e crittografate",
        ...
        "login-register-note": "L'accesso è soggetto a verifica KYC/AML e alla valutazione dell'idoneità dell'investitore. Il nostro team ti contatterà entro 24-48 ore.",
    ...
    es: {
        ...
        "hero-lead": "EquusChain es la infraestructura moderna para la propiedad, el intercambio y la liquidez de activos reales excepcionales y finanzas tradicionales.",
        "manifesto-tag": "I. Nuestra Convicción",
        "manifesto-body": "Los activos más duraderos del mundo siempre han trascendido los mercados y las generaciones. Desde empresas privadas hasta bienes raíces excepcionales, pasando por caballos de carreras distinguidos y bellas artes, su valor va más allá del rendimiento financiero por sí solo. EquusChain proporciona la infraestructura para gestionar, intercambiar y desbloquear la liquidez de estos activos con la discreción, transparencia y eficiencia que merecen.",
        "universe-subtitle": "II. Universo de activos", 
        "universe-title": "Universo de activos<br>excepcionales de EquusChain.",
        "universe-lead": "Un universo amplio y en crecimiento. Desde mercados privados institucionales hasta los activos reales más codiciados del mundo. A continuación una selección, el catálogo completo está dentro de la plataforma.",
        "asset-1-name": "Bienes Raíces e Infraestructura", 
        "asset-1-sub": "Fraccionado o completo.",
        ...
        "asset-8-sub": "Metales preciosos, fondos estructurados, energía renovable…",
        "services-subtitle": "III. Servicios", 
        "services-title": "Lo que la plataforma<br>EquusChain permite.",
        "services-lead": "Cuatro servicios. Cada uno diseñado para un tipo de relación específica con el capital. Puede necesitar uno o los cuatro.",
        "service-1-n": "I", 
        "service-1-title": "Invertir y Negociar", 
        "service-1-body": "Suscríbase a ofertas. Gestione posiciones. Reciba distribuciones. Un entorno privado único para una cartera que se extiende más allá de los mercados cotizados.", 
        "service-1-link": "Abrir una cuenta",
        "service-2-n": "II", 
        "service-2-title": "Propiedad Fraccionada", 
        "service-2-body": "Tenga participaciones en activos que de otro modo requerirían compromisos de capital exclusivo. Bienes raíces, empresas privadas, caballos, aviones, y más; al tamaño que se adapte a su mandato.", 
        "service-2-link": "Preguntar",
        "service-3-n": "III", 
        "service-3-title": "Introducir activos al mercado", 
        "service-3-body": "Usted posee el activo. Nosotros lo estructuramos, documentamos e introducimos a inversores cualificados. Desde su empresa privada hasta una cartera inmobiliaria o un yate; todo el proceso gestionado.", 
        "service-3-link": "Hable con nuestro equipo",
        "service-4-n": "IV", 
        "service-4-title": "Derechos de acceso y uso", 
        "service-4-body": "Ciertos activos de la plataforma están disponibles a través de acuerdos de acceso preferencial. Aviación, marítimo, ecuestre. Reservado para miembros.", 
        "service-4-link": "Explorar",
        "contact-subtitle": "IV. Access", 
        "contact-title": "Solicitar<br><em>Acceso.</em>",
        "contact-body": "Cada solicitud es revisada directamente por nuestro equipo para asegurar la calidad, relevancia e integridad del ecosistema de EquusChain.",
        ...
        "form-newsletter-text": "Deseo suscribirme al boletín de noticias de EquusChain", 
        "form-consent-text": "Confirmo que he leído y acepto los <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Términos y Condiciones</a> y la <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Política de Privacidad</a>", 
        "form-submit-btn": "Enviar solicitud", 
        "form-note-footer": "Su solicitud es tratada con absoluta discreción. No compartimos sus datos con terceros.",
        ...
        "ft-disclaimer": "© 2026 EquusChain Ltd. Todos los derechos reservados. La información de este sitio web es puramente informativa y no constituye asesoramiento financiero, legal o fiscal, ni una oferta de compra o venta de ningún instrumento financiero o activo.",
        "ft-privacy": "Privacidad", 
        "ft-terms": "Términos",
        "cookie-title": "Usamos cookies", 
        "cookie-body": "Utilizamos cookies para mejorar la experiencia, analizar el uso del sitio y ofrecer contenido personalizado. Algunas cookies son esenciales para el funcionamiento (seguridad, sesión). Otras requieren su consentimiento previo.",
        ...
        "login-note": "Acceso protegido · Todas las sesiones están siendo monitoreadas y cifradas",
        ...
        "login-register-note": "El acceso está sujeto a verificación KYC/AML y a la evaluación de elegibilidad del inversor. Nuestro equipo se pondrá en contacto en un plazo de 24 a 48 horas.",
    ...
    de: {
        ...
        "hero-lead": "EquusChain ist die moderne Infrastruktur für Eigentum, Austausch und Liquidität außergewöhnlicher Real-World- und traditioneller Finanzwerte.",
        "manifesto-tag": "I. Unsere Überzeugung",
        "manifesto-body": "Die beständigsten Sachwerte der Welt haben schon immer Märkte und Generationen überdauert. Von Familienunternehmen über exklusive Immobilien bis hin zu edlem Vollblut und bildender Kunst — ihr Wert geht weit über die finanzielle Rendite hinaus. EquusChain bietet die Infrastruktur, um diese Werte mit der Diskretion, Transparenz und Effizienz zu verwalten und liquide zu machen, die sie verdienen.",
        "universe-subtitle": "II. Anlageuniversum", 
        "universe-title": "EquusChain Universum<br>außergewöhnlicher Sachwerte.",
        "universe-lead": "Ein weitreichendes und wachsendes Universum. Von institutionellen privaten Märkten bis zu den begehrtesten Sachwerten der Welt. Nachfolgend eine Auswahl, das vollständige Portfolio befindet sich auf der Plattform.",
        "asset-1-name": "Immobilien & Infrastruktur", 
        "asset-1-sub": "Fraktioniert oder als Ganzes.",
        "asset-2-name": "Private Equity", 
        "asset-2-sub": "Fonds, Co-Investments und Direktbeteiligungen.",
        "asset-3-name": "Private Debt", 
        "asset-3-sub": "Erstrangig besicherte Kredite und maßgeschneiderte Kreditstrukturen.",
        ...
        "services-lead": "Vier Säulen. Jede für eine spezifische Beziehung zu Kapital konzipiert. Sie benötigen eventuell eine oder alle vier.",
        "service-1-n": "I", 
        "service-1-title": "Investieren & Handeln", 
        "service-1-body": "Zeichnen Sie Angebote. Verwalten Sie Positionen. Erhalten Sie Ausschüttungen. Eine geschlossene Umgebung für ein Portfolio abseits der Börsenmärkte.", 
        "service-1-link": "Konto eröffnen",
        "service-2-n": "II", 
        "service-2-title": "Miteigentum (Fractional)", 
        "service-2-body": "Halten Sie Anteile an Werten, die sonst exklusives Kapital erfordern würden. Immobilien, Unternehmen, Rennpferde, Flugzeuge — in der für Ihr Mandat passenden Größe.", 
        "service-2-link": "Anfragen",
        "service-3-n": "III", 
        "service-3-title": "Sachwerte auf den Markt bringen", 
        "service-3-body": "Sie halten das Asset. Wir strukturieren, dokumentieren und vermitteln es an qualifizierte Anleger. Vom Familienunternehmen bis hin zum Immobilienportfolio oder einer Yacht.", 
        "service-3-link": "Sprechen Sie mit unserem Team",
        "service-4-n": "IV", 
        "service-4-title": "Nutzungs- & Zugangsrechte", 
        "service-4-body": "Bestimmte Plattform-Assets sind über bevorzugte Nutzungsvereinbarungen zugänglich. Luftfahrt, Schifffahrt, Pferdesport. Reserviert für Mitglieder.", 
        "service-4-link": "Erkunden",
        "contact-subtitle": "IV. Zugang", 
        "contact-title": "Zugang<br><em>anfordern.</em>",
        "contact-body": "Jede Anfrage wird direkt von unserem Team geprüft, um die Qualität, Relevanz und Integrität des EquusChain-Ökosystems zu gewährleisten.",
        ...
        "form-newsletter-text": "Ich möchte den kuratierten Newsletter von EquusChain abonnieren", 
        "form-consent-text": "Ich bestätige, dass ich die <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Allgemeinen Geschäftsbedingungen</a> und die <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Datenschutzerklärung</a> gelesen habe und ihnen zustimme", 
        "form-submit-btn": "Anfrage senden", 
        "form-note-footer": "Ihre Anfrage wird mit absoluter Diskretion behandelt. Wir geben keine Daten an Dritte weiter.",
        ...
        "ft-disclaimer": "© 2026 EquusChain Ltd. Alle Rechte vorbehalten. Die Informationen auf dieser Website dienen ausschließlich Informationszwecken und stellen keine Anlageberatung dar. Investitionen bergen Risiken bis hin zum vollständigen Kapitalverlust.",
        "ft-privacy": "Datenschutz", 
        "ft-terms": "Bedingungen",
        "cookie-title": "Wir verwenden Cookies", 
        "cookie-body": "Wir verwenden Cookies, um Ihre Nutzererfahrung zu verbessern, die Website-Nutzung zu analysieren und Ihnen personalisierte Angebote zu unterbreiten. Einige Cookies sind essenziell, andere erfordern Ihre Zustimmung.",
        ...
        "login-note": "Geschätzter Zugang · Alle Sitzungen werden überwacht und verschlüsselt",
        ...
        "login-register-note": "Der Zugang unterliegt der KYC/AML-Prüfung und der Beurteilung der Anlegereignung. Unser Team wird sich innerhalb von 24–48 Stunden mit Ihnen in Verbindung setzen.",
    ...
    pt: {
        ...
        "hero-lead": "EquusChain é a infraestrutura moderna para a propriedade, negociação e liquidez de ativos reais excepcionais e finanças tradicionais.",
        "manifesto-tag": "I. Nossa Convicção",
        "manifesto-body": "Os ativos mais duradouros do mundo sempre transcenderam mercados e gerações. De empresas privadas a imóveis excepcionais, passando por cavalos puro-sangue e belas artes, seu valor vai além do desempenho financeiro. A EquusChain fornece a infraestrutura para gerir, negociar e desbloquear liquidez nestes ativos com a discrição, transparência e eficiência que merecem.",
        "universe-subtitle": "II. Universo de ativos", 
        "universe-title": "Universo de ativos<br>excecionais EquusChain.",
        "universe-lead": "Um universo amplo e em crescimento. De mercados privados institucionais aos ativos reais mais cobiçados do mundo. Uma seleção abaixo, o catálogo completo está na plataforma.",
        "asset-1-name": "Imobiliário & Infraestrutura", 
        "asset-1-sub": "Fracionado ou inteiro.",
        "asset-2-name": "Private Equity", 
        ...
        "asset-5-sub": "Cavalos de corrida, direitos de cobertura, programas hípicos.",
        ...
        "services-lead": "Quatro soluções. Cada uma desenhada para um tipo de relação com o capital. Você pode precisar de uma ou de todas.",
        "service-1-n": "I", 
        "service-1-title": "Investir & Negociar", 
        "service-1-body": "Subscreva ofertas. Gira posições. Receba distribuições. Um ecossistema privado para uma carteira que vai muito além dos mercados cotados em bolsa.", 
        "service-1-link": "Abrir uma conta",
        "service-2-n": "II", 
        "service-2-title": "Proprietà Frazionata", 
        "service-2-body": "Detenha frações de ativos que de outra forma exigiriam aportes exclusivos de capital. Imóveis, empresas privadas, cavalos, aviões; no tamanho certo para o seu mandato.", 
        "service-2-link": "Consultar",
        "service-3-n": "III", 
        "service-3-title": "Trazer ativos ao mercado", 
        "service-3-body": "Você tem o ativo. Nós o estruturamos, documentamos e apresentamos a investidores qualificados. Da sua empresa privada a um portfólio de imóveis ou iate; todo o processo gerido.", 
        "service-3-link": "Fale com nossa equipe",
        "service-4-n": "IV", 
        "service-4-title": "Direitos de Acesso & Uso", 
        "service-4-body": "Certos ativos da plataforma estão disponíveis por acordos de acesso preferencial. Aviação, marítimo, hípico. Reservado a membros.", 
        "service-4-link": "Explorar",
        "contact-subtitle": "IV. Acesso", 
        "contact-title": "Solicitar<br><em>Acesso.</em>",
        "contact-body": "Cada solicitação é revista diretamente pela nossa equipa para assegurar a qualidade, relevância e integridade do ecossistema EquusChain.",
        "form-name-label": "Nome completo", 
        ...
        "form-consent-text": "Confirmo que li e aceito os <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Termos e Condições</a> e a <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Política de Privacidade</a>", 
        "form-submit-btn": "Enviar pedido", 
        "form-note-footer": "O seu pedido é tratado com discrição absoluta. Não partilhamos os seus dados com terceiros.",
        "ft-col1-title": "Plataforma", 
        ...
        "ft-disclaimer": "© 2026 EquusChain Ltd. Todos os direitos reservados. As informações contidas neste website são fornecidas apenas para fins informativos e não constituem aconselhamento de investimento, jurídico ou fiscal.",
        "ft-privacy": "Privacidade", 
        "ft-terms": "Termos",
        "cookie-title": "Usamos cookies", 
        "cookie-body": "Usamos cookies para melhorar sua experiência, analisar o uso do site e oferecer conteúdo e ofertas personalizadas. Alguns cookies são essenciais, outros requerem consentimento prévio.",
        ...
        "login-note": "Acesso protegido · Todas as sessões são monitoradas e criptografadas",
        ...
        "login-register-note": "O acesso está sujeito à verificação de KYC/AML e à avaliação de elegibilidade do investidor. A nossa equipa entrará em contacto num prazo de 24–48 horas.",
    ...
    ru: {
        ...
        "hero-lead": "EquusChain — современная инфраструктура для владения, обмена и обеспечения ликвидности исключительных реальных активов и традиционных финансов.",
        "manifesto-tag": "I. Наше кредо",
        "manifesto-body": "Самые ценные активы в мире всегда превосходили рынки и поколения. От частных предприятий до элитной недвижимости, выдающегося племенного поголовья и предметов искусства — их ценность выходит за рамки только лишь финансовых показателей. EquusChain предоставляет инфраструктуру для управления, обмена и высвобождения ликвидности этих активов с конфиденциальностью, прозрачностью и эффективностью, которых они заслуживают.",
        "universe-subtitle": "II. Вселенная активов", 
        "universe-title": "Вселенная исключительных<br>активов EquusChain.",
        "universe-lead": "Обширная и растущая вселенная. От институциональных частных рынков до самых желанных реальных активов в мире. Краткий перечень ниже, полный каталог доступен на платформе.",
        "asset-1-name": "Недвижимость и инфраструктура", 
        "asset-1-sub": "Долевое или полное владение.",
        "asset-2-name": "Прямые инвестиции", 
        "asset-2-sub": "Фонды, ко-инвестиции и прямые позиции.",
        "asset-3-name": "Частное кредитование", 
        "asset-3-sub": "Старший обеспеченный долг и индивидуальные кредитные структуры.",
        "asset-4-name": "Сельское хозяйство и сырье", 
        "asset-4-sub": "Продуктивные земли и инструменты, связанные с сырьевыми товарами.",
        ...
        "asset-7-sub": "Произведения музейного уровня, редкие вина, классические автомобили.",
        "asset-8-name": "И многое другое", 
        "asset-8-sub": "Благородные металлы, структурированные фонды, возобновляемая энергия…",
        "services-subtitle": "III. Услуги", 
        "services-title": "Что позволяет делать<br>платформа EquusChain.",
        "services-lead": "Четыре направления. Каждое создано для особых отношений с капиталом. Вам может потребоваться одно или все четыре.",
        "service-1-n": "I", 
        "service-1-title": "Инвестиции и трейдинг", 
        "service-1-body": "Подписывайтесь на предложения. Управляйте позициями. Получайте распределения прибыли. Единое закрытое пространство для портфеля вне биржевого рынка.", 
        "service-1-link": "Открыть счет",
        "service-2-n": "II", 
        "service-2-title": "Долевое владение", 
        "service-2-body": "Владейте долями в активах, которые иначе потребовали бы единоличных крупных инвестиций. Недвижимость, частные компании, коневодство, самолеты; в объемах под ваш мандат.", 
        "service-2-link": "Узнать больше",
        "service-3-n": "III", 
        "service-3-title": "Вывод активов на рынок", 
        "service-3-body": "Вы владеете активом. Мы его структурируем, документируем и выводим к квалифицированным инвесторам. От вашей частной компании до пула недвижимости или яхты.", 
        "service-3-link": "Связаться с командой",
        "service-4-n": "IV", 
        "service-4-title": "Права доступа и использования", 
        "service-4-body": "Некоторые активы платформы доступны по соглашениям о преференциальном доступе. Авиация, флот, конный спорт. Только для членов клуба.", 
        "service-4-link": "Исследовать",
        "contact-subtitle": "IV. Доступ", 
        "contact-title": "Запросить<br><em>Доступ.</em>",
        "contact-body": "Каждый запрос рассматривается нашей командой напрямую, чтобы гарантировать качество, релевантность и целостность экосистемы EquusChain.",
        ...
        "form-newsletter-text": "Я хочу подписаться на рассылку новостей EquusChain", 
        "form-consent-text": "Я подтверждаю, что прочитал и согласен с <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">Условиями использования</a> и <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">Политикой конфиденциальности</a>", 
        "form-submit-btn": "Отправить запрос", 
        "form-note-footer": "Ваш запрос обрабатывается со строгой конфиденциальностью. Мы не передаем данные третьим лицам.",
        ...
        "ft-disclaimer": "© 2026 EquusChain Ltd. Все права защищены. Информация на данном веб-сайте носит исключительно ознакомительный характер и не является инвестиционной, юридической или налоговой консультацией.",
        "ft-privacy": "Конфиденциальность", 
        "ft-terms": "Правила",
        "cookie-title": "Мы используем файлы cookie", 
        "cookie-body": "Мы используем файлы cookie для улучшения работы сайта, анализа трафика и показа персонализированных предложений. Некоторые файлы cookie обязательны для безопасности.",
        ...
        "login-register-note": "Доступ предоставляется при условии прохождения верификации KYC/AML и оценки статуса инвестора. Наша команда свяжется с вами в течение 24–48 часов.",
    ...
    zh: {
        ...
        "hero-lead": "EquusChain 是面向非凡真实世界与传统金融资产的所有权、交易 and 和流动性的现代基础设施。",
        "manifesto-tag": "I. 我们的信念",
        "manifesto-body": "世界上最持久的资产向来超越市场与时代。从私有企业到顶级房地产，再到优良血统马匹和美术品，它们的价值早已超越了纯粹的财务表现。EquusChain 提供了专业的基础设施，以其应得的谨慎、透明和高效来管理、交易并释放这些资产的流动性。",
        "universe-subtitle": "II. 资产星系", 
        "universe-title": "EquusChain 非凡资产<br>的庞大宇宙。",
        "universe-lead": "一个广泛且不断成长的宇宙。从机构私募市场到全球最令人向往的实物资产。精选部分见下方，完整目录在平台内部。",
        "asset-1-name": "房地产与基础设施", 
        "asset-1-sub": "拆分或整体所有权。",
        "asset-2-name": "私募股权", 
        ...
        "services-lead": "四大维度。每一个都专为特定的资本关系而设计。您可能需要其中一个，也可能需要全部。",
        "service-1-n": "I", 
        "service-1-title": "投资与交易", 
        "service-1-body": "认购份额。管理仓位。获取分配收益。在一个私密环境内管理远超上市公开市场的投资组合。", 
        "service-1-link": "开立账户",
        "service-2-n": "II", 
        "service-2-title": "碎片化所有权", 
        "service-2-body": "持有原本需要大额独占资本的资产权益。房地产、私营企业、血统马匹、航空器等；以契合您的投资授权的规模进行配置。", 
        "service-2-link": "业务咨询",
        "service-3-n": "III", 
        "service-3-title": "资产推向市场", 
        "service-3-body": "您持有资产，我们来进行结构设计、文件编制并引荐给合格投资者。从私营公司到地产组合或游艇，全流程托付。", 
        "service-3-link": "联系我们的团队",
        "service-4-n": "IV", 
        "service-4-title": "特权访问与使用权", 
        "service-4-body": "平台内的特定资产可通过优先协议访问。航空、航海、马术。专为会员保留。", 
        "service-4-link": "立即探索",
        "contact-subtitle": "IV. 访问", 
        "contact-title": "申请<br><em>访问。</em>",
        "contact-body": "我们团队会直接审核每一份申请，以维护 EquusChain 生态系统的品质、关联度与完整性。",
        "form-name-label": "姓名", 
        ...
        "form-consent-text": "我确认已阅读并同意<a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">条款与条件</a>以及<a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">隐私政策</a>", 
        "form-submit-btn": "提交申请", 
        "form-note-footer": "您的所有咨询均受严格保密。我们不会与任何第三方共享信息。",
        "ft-col1-title": "平台入口", 
        "ft-col1-item1": "会员登录", 
        "ft-col1-item2": "资产世界", 
        "ft-col1-item3": "资产上架",
        ...
        "ft-col2-item2": "机构业务部", 
        "ft-col2-item3": "法务与合规",
        "ft-disclaimer": "© 2026 EquusChain Ltd. 保留所有权利。本网站所载信息仅供参考，不构成任何投资、法律、税务或财务建议，亦不构成购买或出售任何证券或资产的要约。",
        "ft-privacy": "隐私政策", 
        "ft-terms": "使用条款",
        "cookie-title": "我们使用 Cookie", 
        "cookie-body": "我们使用 Cookie 来优化用户体验、分析网站流量以及提供个性化内容。某些 Cookie 对于网站的运行必不可少，而其他 Cookie 则需获得您的事先同意。",
        ...
        "login-register-note": "平台访问权限需经KYC/AML合规性审查及投资者资格评估。我们的团队将在24-48小时内与您联系。",
    ...
    hi: {
        ...
        "hero-lead": "इक्वसचेन असाधारण वास्तविक और पारंपरिक वित्तीय संपत्तियों के स्वामित्व, विनिमय और तरलता के लिए आधुनिक बुनियादी ढांचा है।",
        "manifesto-tag": "I. हमारा विश्वास",
        "manifesto-body": "दुनिया की सबसे स्थायी संपत्तियों ने हमेशा बाजारों और पीढ़ियों को पार किया है। निजी उद्यमों से लेकर असाधारण अचल संपत्ति तक, प्रतिष्ठित घोड़ों और ललित कला तक, उनका मूल्य केवल वित्तीय प्रदर्शन से परे है। इक्वसचेन इन संपत्तियों में गोपनीयता, पारदर्शिता और दक्षता के साथ तरलता का प्रबंधन, विनिमय और अनलॉक करने के लिए बुनियादी ढांचा प्रदान करता है।",
        "universe-subtitle": "II. संपत्ति ब्रह्मांड", 
        "universe-title": "इक्वसचेन का असाधारण<br>संपत्ति ब्रह्मांड।",
        "universe-lead": "एक विस्तृत और बढ़ता हुआ ब्रह्मांड। संस्थागत निजी बाजारों से लेकर दुनिया की सबसे प्रतिष्ठित संपत्तियों तक। नीचे एक चयन है, पूरी सूची मंच के भीतर है।",
        "asset-1-name": "रियल एस्टेट और बुनियादी ढांचा", 
        "asset-1-sub": "आंशिक या संपूर्ण।",
        ...
        "asset-5-name": "घुड़सवारी और रक्तपात", 
        ...
        "services-lead": "चार चीजें। प्रत्येक पूंजी के साथ एक विशिष्ट संबंध के लिए डिज़ाइन की गई है। आपको एक या चारों की आवश्यकता हो सकती है।",
        "service-1-n": "I", 
        "service-1-title": "निवेश और व्यापार", 
        "service-1-body": "प्रसादों की सदस्यता लें। पदों का प्रबंधन करें। वितरण प्राप्त करें। एक निजी वातावरण उस पोर्टफोलियो के लिए जो सार्वजनिक बाजारों से बहुत आगे जाता है।", 
        "service-1-link": "खाता खोलें",
        "service-2-n": "II", 
        "service-2-title": "आंशिक स्वामित्व", 
        "service-2-body": "उन संपत्तियों में रुचि रखें जो अन्यथा विशेष पूंजी प्रतिबद्धता की मांग करती हैं। रियल एस्टेट, निजी उद्यम, घोड़े, विमान, and और बहुत कुछ; आपके जनादेश के आकार में।", 
        "service-2-link": "पूछताछ करें",
        "service-3-n": "III", 
        "service-3-title": "संपत्तियों को बाजार में लाना", 
        "service-3-body": "आप संपत्ति रखते हैं। हम इसे संरचित करते हैं, इसका दस्तावेजीकरण करते हैं, और इसे योग्य निवेशकों के पास लाते हैं। आपकी निजी कंपनी से लेकर संपत्ति पोर्टफोलियो या नाव तक; पूरी प्रक्रिया प्रबंधित की जाती है।", 
        "service-3-link": "हमारी टीम से बात करें",
        "service-4-n": "IV", 
        "service-4-title": "पहुंच और उपयोग के अधिकार", 
        "service-4-body": "कुछ प्लेटफॉर्म संपत्तियां अधिमान्य पहुंच व्यवस्थाओं के माध्यम से उपलब्ध हैं। विमानन, समुद्री, घुड़सवारी। सदस्यों के लिए आरक्षित।", 
        "service-4-link": "अन्वेषण करें",
        "contact-subtitle": "IV. पहुंच", 
        "contact-title": "अनुरोध<br><em>पहुंच।</em>",
        "contact-body": "इक्वसचेन पारिस्थितिकी तंत्र की गुणवत्ता, प्रासंगिकता और अखंडता सुनिश्चित करने के लिए हमारी टीम द्वारा सीधे हर पूछताछ की समीक्षा की जाती है।",
        ...
        "form-newsletter-text": "मैं इक्वसचेन के क्यूरेटेड न्यूजलेटर की सदस्यता लेना चाहूंगा", 
        "form-consent-text": "मैं पुष्टि करता हूं कि मैंने <a href=\"#termsModal\" class=\"live-link\" id=\"termsLink\">नियम और शर्तें</a> और <a href=\"#privacyModal\" class=\"live-link\" id=\"privacyLink\">गोपनीयता नीति</a> पढ़ ली है और उनसे सहमत हूं", 
        "form-submit-btn": "अनुरोध जमा करें", 
        "form-note-footer": "आपकी पूछताछ पूर्ण गोपनीयता के साथ संभाली जाती है। हम तीसरे पक्षों के साथ विवरण साझा नहीं करते हैं।",
        "ft-col1-title": "मंच", 
        ...
        "ft-disclaimer": "© 2026 इक्वसचेन लिमिटेड। सर्वाधिकार सुरक्षित। इस वेबसाइट पर दी गई जानकारी केवल सूचनात्मक उद्देश्यों के लिए प्रदान की गई है और यह निवेश, कानूनी, कर या वित्तीय सलाह का गठन नहीं करती है।",
        "ft-privacy": "गोपनीयता", 
        "ft-terms": "नियम",
        "cookie-title": "हम कुकीज़ का उपयोग करते हैं", 
        "cookie-body": "हम आपके अनुभव को बेहतर बनाने, साइट उपयोग का विश्लेषण करने और व्यक्तिगत सामग्री प्रदान करने के लिए कुकीज़ का उपयोग करते हैं। कुछ कुकीज़ आवश्यक हैं, अन्य के लिए आपकी सहमति आवश्यक है।",
        ...
        "login-note": "सुरक्षित पहुंच · सभी सत्रों की निगरानी और एन्क्रिप्शन किया जाता है",
        ...
        "login-register-note": "पहुंच केवाईसी/एएमएल सत्यापन और निवेशक पात्रता मूल्यांकन के अधीन है। हमारी टीम 24-48 घंटों के भीतर संपर्क करेगी।",
    ...
    ja: {
        ...
        "hero-lead": "EquusChainは、卓越した実物資産および伝統的金融資産の所有、取引、流動化のためのモダンなインフラストラクチャです。",
        "manifesto-tag": "I. 私たちの信念",
        "manifesto-body": "世界で最も永続的な資産は、常に市場や世代を超越してきました。私企業から優れた不動産、一流の競走馬、そして美術品に至るまで、その価値は財務パフォーマンスのみに留まりません。EquusChainは、これらの資産の流動性を、相応しい配慮、透明性、効率性をもって管理、取引、解放するためのインフラを提供します。",
        "universe-subtitle": "II. 資産の宇宙",
        "universe-title": "EquusChainが誇る<br>卓越した資産の宇宙。",
        "universe-lead": "広大で成長を続ける宇宙。機関投資家向けプライベート市場から世界で最も切望される実物資産まで。以下はその一部です。全カタログはプラットフォーム内にあります。",
        "asset-1-name": "不動産・インフラ",
        "asset-1-sub": "小口化または全体所有。",
        ...
        "services-lead": "4つの主要サービス。それぞれ特定の資本関係に合わせて設計されています。1つだけ必要な場合も、4つすべてが必要な場合もあります。",
        "service-1-n": "I",
        "service-1-title": "投資と取引",
        "service-1-body": "案件への応募、ポジション管理、分配金の受領。上場市場をはるかに超えるポートフォリオのためのプライベート環境。",
        "service-1-link": "アカウントを開設する",
        "service-2-n": "II",
        "service-2-title": "小口所有権",
        "service-2-body": "本来であれば独占的な資本コミットメントが必要な資産の持分を保有。不動産、私企業、競走馬、航空機など、お客様の投資方針に応じた規模で。",
        "service-2-link": "お問い合わせ",
        "service-3-n": "III",
        "service-3-title": "資産の市場投入",
        "service-3-body": "お客様は資産を保有するだけ。当方がストラクチャリング、ドキュメンテーションを行い、適格投資家に紹介します。非公開企業から不動産ポートフォリオ、ヨットまで、すべてのプロセスをサポート。",
        "service-3-link": "チームに相談する",
        "service-4-n": "IV",
        "service-4-title": "アクセスおよび使用権",
        "service-4-body": "プラットフォームの特定の資産は、優先アクセス契約を通じてご利用いただけます。航空、海洋、馬術。会員専用。",
        "service-4-link": "探索する",
        "contact-subtitle": "IV. アクセス",
        "contact-title": "アクセスを<br><em>申請する。</em>",
        "contact-body": "すべての問い合わせは、EquusChainエコシステムの品質、関連性、完全性を担保するため、当社のチームが直接審査いたします。",
        "form-name-label": "氏名",
        ...
        "form-note-footer": "お問い合わせ内容は完全に極秘として扱われます。第三者と詳細を共有することはありません。",
        ...
        "ft-disclaimer": "© 2026 EquusChain Ltd. 無断複写・転載を禁じます。本ウェブサイトに掲載されている情報は情報提供のみを目的としており、投資、法律、税務、財務に関するアドバイスを構成するものではありません。",
        "ft-privacy": "プライバシー",
        "ft-terms": "利用規約",
        "cookie-title": "クッキーを使用しています",
        "cookie-body": "当サイトでは、ユーザー体験の向上、サイト利用状況の分析、パーソナライズされたコンテンツや提案の提供のためにクッキーを使用しています。一部のクッキーは必須ですが、その他は事前の同意が必要です。",
        ...
        "login-note": "保護されたアクセス · すべてのセッションは監視および暗号化されます",
        ...
        "login-register-note": "アクセスにはKYC/AMLの確認および投資家要件の審査が必要となります。当社のチームより24〜48時間以内にご連絡いたします。",
...
function switchLanguage(lang) {
    const langCurr = document.getElementById('langBtn') ? document.getElementById('langBtn').querySelector('.lang-curr') : null;
    if (langCurr) {
    ...
    if (dateEl) {
        const locales = {
            en: 'en-GB', fr: 'fr-FR', ar: 'ar-AE', it: 'it-IT', es: 'es-ES',
            de: 'de-DE', pt: 'pt-PT', ru: 'ru-RU', zh: 'zh-CN', hi: 'hi-IN', ja: 'ja-JP'
        };
        const locale = locales[lang] || 'en-GB';
        const d = new Date();
        dateEl.textContent = d.toLocaleDateString(locale, {day:'numeric',month:'long',year:'numeric'});
    }
...
if (topbarDateGlobal) {
    const d = new Date();
    topbarDateGlobal.textContent = d.toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'});
}
...
function wizNext() { wizStep(wizCurrent + 1); }

// Tokenise Asset Submission
function submitTokenisation() {
    let msg = 'Your tokenisation application has been submitted.\n\nOur structuring team will review your submission and contact you within 48 hours with an initial assessment.\n\nThank you.';
    if (window.currentLangDict && window.currentLangDict['alert-tokenisation-submitted']) {
